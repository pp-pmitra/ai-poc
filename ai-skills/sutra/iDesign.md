...
- **Writes (STEP 6):** `mcp__github__create_branch` (branch: `Sutra_NNN`, from_branch: `QA-1793`) → `mcp__github__create_or_update_file` (or `push_files` for multiple files) on that branch → `mcp__github__create_pull_request` (base: `QA-1793`, head: `Sutra_NNN`). PR URL from the tool's own response — never fabricated. Branch index `NNN`: list existing branches/PRs for the highest `Sutra_NNN` and increment; on ambiguity, err high rather than collide.
...
- **Branch:** find the highest existing `Sutra_NNN` — `git fetch --all` then `git branch -r | grep Sutra_` on the Local-Checkout Path, or listing branches/PRs via `mcp__github__*` on the GitHub-Connector Path — and create `Sutra_<NNN+1>`, based off `QA-1793` (not the repo's git-default branch).
...
- **PR:** `gh pr create --title "<title>" --body "$(cat <<'EOF' ... EOF)"` on the Local-Checkout Path, or `mcp__github__create_pull_request` (base: `QA-1793`, head: `Sutra_<NNN+1>`) on the GitHub-Connector Path — against the target branch either way. Body follows this exact template, section for section, in order — never drop/merge/reorder for brevity:
