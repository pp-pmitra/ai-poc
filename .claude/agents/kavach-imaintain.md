...
tools:
  ...
  - Write(.claude/skills/kavach-diagnose/history/fix-history.json)
...
- `Write`/`Edit` are scoped to exactly the files the kavach-diagnose skill names as ever-editable: `.feature` files, `src/test/java/stepdefinitions/`, `src/main/java/pages/`, plus this agent's own `fix-history.json` append and its `kavach-knowledge` fix-pattern append. Never application source outside `pages/`, never pattern files by hand beyond the documented appends.
...
Follow the `kavach-imaintain` skill (`.claude/skills/kavach-imaintain/SKILL.md`) in full and exactly — its phases (candidate selection, per-fix application, three-run Maven verification, commit/PR, fix-history update) are the canonical procedure and are not duplicated here.
