...
**TOOL USE: Bash, Read, Write and Edit (both scoped to `kavach-data/**` only — never a bare `Write(*)`/`Edit(*)`, see the worked `--allowedTools` examples below), Grep/Glob, and Playwright MCP browser tools are pre-approved for this skill when it is run with `--permission-mode auto` or `--permission-mode bypassPermissions` and the repo's `allowedTools` are configured. Note that `Bash(*)` remains unrestricted in every example below — the `Write`/`Edit` scoping is not a real containment boundary on its own, since a shell redirection could still write anywhere; the actual enforcement backstop is a post-hoc "verify repository boundaries" check (`git status --porcelain` outside `kavach-data/**`/`target/**`) that must run immediately after every invocation of this skill and halt the run if it finds an out-of-scope write. `.github/workflows/kavach.yml`'s "Verify repository boundaries" step is this check for the CI path, gating the downstream PR job on its outcome; `.claude/run-mixed-batch.sh`'s `verify_boundaries()` is the same check for the manual path. Do not invoke this skill from any entry point that skips this check. Do not infer that Playwright is unavailable from the absence of an interactive prompt in `claude -p`. Do not ask the user to approve `mcp__playwright__browser_*` tools. Do not write "the browser tool needs permission", "please approve", "while waiting", or any static-only fallback before a real tool rejection. Call the required Playwright MCP tool directly. If Claude Code actually rejects the tool call (permission-model rejection), stop immediately and print exactly: `PLAYWRIGHT_TOOL_REJECTED: <tool name>: <error text>`. If instead the call fails with a connection-refused or "Target closed" error (dead CDP endpoint), follow the `CDP_ENDPOINT_DEAD` procedure in `_default.md`'s Bootstrap / CDP error handling section. Any such block still ends with a verdict report — see **Live replay blocked** below. Otherwise continue the live replay.**

**FIRST LIVE-REPLAY ACTION:** as soon as the failure-triage skill hands off `escalate-groups.json`, immediately call `mcp__playwright__browser_navigate` to the Background-selected app URL for the first escalated group. Do not write a static-only diagnosis for an escalated group before attempting this tool call — a static-only diagnosis is not a valid final answer for a group failure-triage could not resolve. (Groups failure-triage *did* resolve — mechanically intermittent, fix-pattern cache hit, deterministic, or text-only-LLM-resolved — correctly get no browser session at all; that is the intended cost saving, not a shortcut to flag.)
...
python3 .claude/skills/kavach-diagnose/scripts/replay_workers.py --write-runner --only-groups kavach-data/history/triage-results/<triage-timestamp>/escalate-groups.json \
  ...
  --claude-arg --allowedTools --claude-arg "Bash(*),Read,Write(kavach-data/**),Edit(kavach-data/**),Grep,Glob,mcp__playwright__browser_navigate,mcp__playwright__browser_tabs,mcp__playwright__browser_snapshot,mcp__playwright__browser_evaluate,mcp__playwright__browser_click,mcp__playwright__browser_fill_form,mcp__playwright__browser_type,mcp__playwright__browser_press_key,mcp__playwright__browser_wait_for,mcp__playwright__browser_take_screenshot,mcp__playwright__browser_close,mcp__playwright__browser_run_code_unsafe,mcp__playwright__browser_select_option,mcp__playwright__browser_hover,mcp__playwright__browser_find"
```
...
```json
{
  ...
  "productBugArtifacts": {
    ...
    "testDataOrEnvironmentRuledOut": "the specific value you searched for and what was found, e.g. \"searched 'AutoSegment747695' in the Demo account — 0 rows returned\" — not a boolean restated as text"
  },
