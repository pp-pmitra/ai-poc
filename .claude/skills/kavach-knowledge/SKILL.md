...
- `kavach-data/fix-patterns/_default.md` — cross-feature patterns (dismiss/wait hardening, nested-child-span locator guard, shadow-DOM CSS-over-XPath guidance, invisible/zero-width character detection, navigation utilities, CDP bootstrap error handling — this list may grow; treat `_default.md` itself as the source of truth). Always read, by both agents, for every candidate — supplements but never overrides a feature-specific file.
- `kavach-data/fix-patterns/<feature-slug>.md` — one file per feature, including a pre-created empty placeholder reserved for a feature that has never yet needed a fix. Structure: a `# <Feature> fix patterns` header — `<Feature>` is the feature file's own stem exactly as it appears in the `.feature` filename (e.g. `Life_CampaignDashboard`, not a humanized "Life Campaign Dashboard") — a `## Run log` line, then dated `### YYYY-MM-DD` entries as they accumulate (this is what real files actually look like today — `## Known good fixes`/`## Learned notes` subheadings are optional refinements *within* a dated entry once a pattern proves reusable across multiple runs, not a separate top-level structure every file must have from day one). **A tracked-but-still-empty placeholder file counts as having no content yet** — when appending to one for the first time, pass `--feature-name` to `append_fix_pattern.py` exactly as for a brand-new file; path-existence alone does not mean a header was ever written.
...
- **Both writers append through the same lock-protected script**, `python3 .claude/skills/kavach-diagnose/scripts/append_fix_pattern.py <feature-slug>` (block text on stdin) — never a direct `Edit`/hand-write of the `.md` file for a new entry. kavach-diagnose and kavach-repair can genuinely append to the same feature file at close to the same time (a CI diagnosis run and a human-run kavach-repair session against the same checkout), and this script holds an exclusive `flock` for the whole read-modify-write so the two serialize instead of one silently losing the other's entry — the same protection `fix-history.json` already had via `append_fix_history.py`. Marking an existing entry `[superseded ...]` (below) is a direct in-place edit to an existing line, not a new append, and is not covered by this script — that operation is rare enough, and low-blast-radius enough (append-only, PR-reviewed, worst case is a stale-but-marked entry momentarily reverted by a race), that it still goes through a direct file edit.
- **Dedup criteria — concrete anchor, not a pure judgment call:** before appending, normalize the candidate entry's broken locator string (collapse whitespace, normalize quote style to `"`, ignore case in element/attribute names but not in literal text values) and its target file path, then compare against every existing entry in the same file using that same normalization. **Same normalized locator + same normalized target file + same fix expression = duplicate, skip appending** (a re-run confirming a known pattern still holds is not new information). **Same normalized locator + same target file + a *different* fix expression = not a duplicate** — append the new entry, and note in it that an earlier entry for the same locator may now be stale (see supersession below), rather than silently dropping either one. A locator that differs only in quote style or whitespace from an existing entry is the *same* locator for this comparison — don't append a near-duplicate over a cosmetic difference alone.
