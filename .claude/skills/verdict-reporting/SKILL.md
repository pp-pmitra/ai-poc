...
**If `append_fix_history.py` exits non-zero, stop and print `PHASE_SCRIPT_FAILED: append_fix_history.py: <stderr>`.** The run is not considered complete until this step succeeds — do not proceed to browser-close/cleanup (Phase 6) or report the run as finished if the batch was rejected, even though Phase 4's report file has already been written. A written report with silently-unrecorded history is a worse outcome than a visibly incomplete run, since nothing else signals that `fix-history.json` didn't actually update.
