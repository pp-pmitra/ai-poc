...
6. **`append_fix_history.py`** — the only writer of `kavach-data/history/fix-history.json`: validates each entry against the same machine-format enums as above, appends under an exclusive file lock, and is idempotent against an identical resubmitted batch. Called by `verdict-reporting` Phase 5.
7. **`record_history.py`, `refresh_glossary.py`, `review_verdicts.py`** — supporting/maintenance utilities (history bookkeeping helpers, glossary refresh, human-driven verdict spot-checking via `review_verdicts.py mark`/`report`) — not part of the per-run critical path above.
